...
function sendCommand(host, port, command, timeoutMs = DEFAULT_TIMEOUT_MS) {
    return new Promise((resolve, reject) => {
        ...
        if (code === undefined) {
            // Named so callers can tell a bad request apart from a transport failure
            const err = new Error(`Unknown Sky command: "${command}"`);
            err.name = 'EUNKNOWNCOMMAND';
            reject(err);
            return;
        }
        ...
        // A peer close (FIN or reset) destroys the socket, which cancels the idle
        // timeout — without this the promise would never settle. No-op after a
        // successful send, because finish() has already set `settled`.
        socket.on('close', () => {
            const err = new Error(`Sky connection closed before command completed ${host}:${port || DEFAULT_PORT}`);
            err.name = 'ECONNCLOSED';
            finish(err);
        });
        socket.on('data', data => {
